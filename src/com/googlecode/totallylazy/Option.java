...
public abstract class Option<A> implements Iterable<A>, Value<A>, Functor<A>, Applicative<A>, Monad<A>, Foldable<A> {
    ...
    public static <T> UnaryOperator<Option<T>> identity(Class<T> aClass) {
        return identity();
    }

    public static <T> UnaryOperator<Option<T>> identity() {
    ...
    public abstract <B> B fold(final B seed, final BiFunction<? super B, ? super A, ? extends B> callable);
