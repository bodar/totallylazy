...
public abstract class UnbalancedSet<T> implements PersistentSet<T> {
    protected final Comparator<T> comparator;

    protected UnbalancedSet(Comparator<T> comparator) {
        this.comparator = comparator;
    }

    public static <T extends Comparable<? super T>> UnbalancedSet<T> empty(Class<T> aClass) {
        return UnbalancedSet.<T>empty();
    }

    public static <T extends Comparable<? super T>> UnbalancedSet<T> empty() {
        return empty(Comparators.<T>ascending());
    }

    public static <T> UnbalancedSet<T> empty(Comparator<T> comparator) {
        return new Empty<T>(comparator);
    }

    public static <T extends Comparable<? super T>> PersistentSet<T> set(T value) {
        return node(value, UnbalancedSet.<T>empty(), UnbalancedSet.<T>empty());
    }

    public static <T> PersistentSet<T> set(T value, Comparator<T> comparator) {
        return node(value, UnbalancedSet.<T>empty(comparator), UnbalancedSet.<T>empty(comparator), comparator);
    }
    ...
    public static <T> PersistentSet<T> set(final Iterable<T> values, Comparator<T> comparator) {
        return sequence(values).fold(UnbalancedSet.<T>empty(comparator), functions.<T, PersistentSet<T>>cons());
    }
    ...
    private static class Empty<T> extends UnbalancedSet<T> {
    ...
    private static class Node<T> extends UnbalancedSet<T> {
