...
//    <B> Self<B> flatMap(Function<? super A, ? extends Self<B>> callable);

    class methods {
        public static <L, R> Either<Seq<L>, Seq<R>> sequenceEs(Iterable<? extends Either<? extends L, ? extends R>> iterable) {
            Seq<L> errors = sequence(iterable).flatMap(Either.functions.<L>leftOption());
            if (!errors.isEmpty()) return left(errors);
            return right(Sequences.<R>flatten(iterable));
        }

        public static <L, R> Either<L, Seq<R>> sequenceE(Iterable<? extends Either<? extends L, ? extends R>> iterable) {
            Option<Either<? extends L, ? extends R>> error = sequence(iterable).find(left);
            if (!error.isEmpty()) return left(error.get().left());
            return right(Sequences.<R>flatten(iterable));
        }

        public static <A> Option<Seq<A>> sequenceO(Iterable<? extends Option<? extends A>> iterable) {
