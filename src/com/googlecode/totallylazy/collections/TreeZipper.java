...
public class TreeZipper<K, V> implements Zipper<Pair<K, V>> {
    ...
    public TreeZipper<K, V> modify(Function<? super TreeMap<K, V>, ? extends TreeMap<K, V>> callable) {
    ...
    public int index() {
        return focus.indexOf(value()) + breadcrumbs.
                ...
                fold(0, (i, breadcrumb) -> i + breadcrumb.other.size() + 1);
    }
    ...
    public static class functions extends TreeMap.functions {
        public static Function<Breadcrumb<?, ?>, Direction> direction = new Function<Breadcrumb<?, ?>, Direction>() {
            @Override
