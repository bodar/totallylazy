...
public class SequenceParser<A> extends BaseParser<Sequence<A>> {
    ...
    public static <A> SequenceParser<A> sequenceOf(final Parser<? extends A> a, final Parser<? extends A> b, final Parser<? extends A> c, final Parser<? extends A> d) {
        return new SequenceParser<A>(Sequences.sequence(a, b, c, d));
    }
    ...
    public static <A> SequenceParser<A> sequenceOf(final Parser<? extends A> a, final Parser<? extends A> b, final Parser<? extends A> c, final Parser<? extends A> d, final Parser<? extends A> e) {
        return new SequenceParser<A>(Sequences.sequence(a, b, c, d, e));
    }

    public static <A> SequenceParser<A> sequenceOf(final Parser<? extends A>... parsers) {
        return new SequenceParser<A>(Sequences.sequence(parsers));
    }
    ...
    @Override
    public Result<Sequence<A>> parse(Sequence<Character> characters) throws Exception {
        Sequence<Character> state = characters;
        ...
        return Success.success(Sequences.sequence(parsed),state);
    }
}
