...
public final class LazyCallable1<T, R> extends Function1<T, R> implements Memory {
    ...
    private final Map<T, Either<Exception, R>> state = new HashMap<T, Either<Exception, R>>();
    ...
    public final R call(T instance) throws Exception{
        synchronized (lock) {
            if (!state.containsKey(instance)) {
                try {
                    state.put(instance, Either.<Exception, R>right(callable.call(instance)));
                } catch (Exception e) {
                    state.put(instance, Either.<Exception, R>left(e));
                }
            }
